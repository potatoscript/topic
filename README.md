...
|                                                                       |                                                                                   |
| --------------------------------------------------------------------- | --------------------------------------------------------------------------------- |
| [医療分野のデジタルヘルスについて](#医療分野のデジタルヘルスについて) | 02 月 23 日                                                                       |
| [IT 人材の不足](#IT人材の不足)                                        | 02 月 23 日                                                                       |
...
```
医療分野は比較的景気の動向に左右されにくい分野とされていて、
かつ昨今のコロナへの対応により、
2022年は医療・健康領域のデジタル化が加速しました。

一か月ほど前のアメリカラスベガスで開催された展示市「CES」でも、
デジタルヘルス関連は出展企業のカテゴリーとして一つの主要な分野になっています。

現在特に目立っているのは、遠隔医療システムなどのクラウドを基盤としたサービス、
そしてそれを可能にするウェアラブルな生体データを測定するデバイスの開発です。

私は10年ほど前から、医療画像情報システムや医療機器の組込ソフトウェア開発の業務に携わる
ことが多かったのですが、
オンプレミスの医療情報サーバの保存データのレコード数が数億、数十億の単位と膨大になり、
そのデータ処理が遅いということでパフォーマンス改善に四苦八苦していました。

ちょうどその頃遠隔診断サービスが検討され始めていた時期で、サーバのクラウド化が進み始めましたが、
現状はまだクラウドに移行した医療機関は全体の数パーセントだそうです。

セキュリティやランニングコストなど課題が多いためだと思われますが、
医療情報のセキュリティガイドラインは国が策定しており、
最近では厚生労働省が主導となった電子処方箋管理サービスの運用が二週間ほど前先月1/26から開始されました。

システムを導入した医療機関や薬局はこのサービスを利用することで、
電子処方箋の発行やそれに基づく調剤が可能になりました。

遠隔診療システムは、５Gの普及につれ、これから数年で一気に進むのではないかと予想されています。

遠隔医療サービスのうち、医師と医師の間の画像診断や病理診断サービスは進んでいますが、
近年は、医師と患者の間の遠隔診療サービスも一般的になってきています。

それを可能にするのは、自宅で患者自身が操作可能な生体データを測定するデバイスで、
その開発も活況を帯びています。
...
スマホのカメラやマイクを使ってデータを取得するスマホアプリあるいはまだ実用化前ですが、
トイレなどもスマートトイレとして、生体データを測定する機器として考えられているようです。

医療分野はそのほか、手術支援ロボットやＡＩ画像診断技術、臨床試験のDXなど、
注目点はいろいろあり、従来の医療業界プレーヤーだけでなく、ＩＴ，電気、通信、
保険業界など様々な企業が入り交じり、動きが活発化しています。
...
```
一昨日の日経クロステックの記事に関して。世界の企業のITの支出が今年6.3%増ということで、
社内の人材の不足がITサービスの追い風になるという記事があり紹介する。

アメリカのガードナーの最新の予測によると2023年はエンタープライズITと呼ばれる企業向けITの世界の支出額が、
昨年に比べて6.3%増の2兆3825億ドル、309兆7215億円になる見通しである。

エンタープライズITと呼ばれているものは、データセンターシステム、サーバーやストレージ、
ネットワーク機器、関連するシステムソフトを含める。

これにソフトウェアITサービスの３分野で、デバイスや通信サービスは含まれない。

ガードナーは、この全てのITの2023年の支出を2.4%増と予測しており、エンタープライズITの伸びを下回っている。
各国の金利の引き上げや企業側の支出の引き締め、場合によっては人員削減をしているが、
このエンタープライズITという支出については増やし続けているとのこと。

ITサービスの支出の増加についてガードナーは、企業の実装とサポートの為に外部スタッフの導入を検討していて、
コンサルティングへの2023年の支出は6.7%増を見込もうとしている。

社内でITの人材を雇用する支出もあるが、ガートナーのCIO(最高情報責任者)は、IT人材の獲得競争に負けていて、
外部のITサービスの支出はあらゆる業界で社内向けよりも急成長しているということで、
スキルの高い人材は賃金要件や開発したいキャリアの向上についていけるテクノロジーサービスプロバイダへ
移行しつつあるとコメントしている。

最後に、今年1月に6.3%増に予測が下がったが、激動時におけるCIOの意思決定も遅れがちではあるが、
エンタープライズITの指数が不況に強いと見ている。コロナ禍に見舞われた2020年の伸び率を振り返ると、
ガードナーは、4月に7.7%、7月に6.9%、10月には4.2%のマイナスの成長を予測し続けた。
これは2020年の話だが、終わってみれば2020年の実績は4.1%増だったということで話を締めている。

この記事を読んで私が思ったのが、最近いわゆるGAFAに人材の削減のニュースがすごく多く取り沙汰されていること、
指数の関係がどうなのかなっていうのがちょっと私もプロではないので分からないのですが、
そういった見解もあるんだいうのが感じたところである。

これが日本の所まで波及しているかというところは分からないが。
少くともわれわれメイテックではIT分野の専門の方がいて、
そちらの分野に強いインフラ系の業務をされている方、あとソフトウェアの業務はもちろんだが、
業務がまだまだ伸びていくという予測があるとしたら、ぜひここにわれわれも追いついて、
しっかり乗っかってやって行ければという様に感じたので、紹介させてもらった。
```
